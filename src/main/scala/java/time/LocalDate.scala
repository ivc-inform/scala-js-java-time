...
final class LocalDate private(year: Int, month: Month, dayOfMonth: Int) extends ChronoLocalDate with Serializable {
    ...
    private lazy val dayOfYear = month.firstDayOfYear(_isLeapYear) + dayOfMonth - 1
    ...
    private lazy val dayOfWeek = Math.floorMod(epochDay + 3, 7).toInt + 1
